...
def reset_grade_calculator():
    ...
    # Reset the actual input boxes to zero
    for subject in [
        "Mathematics",
        "Science",
        "English"
    ]:
        st.session_state[f"grade_score_{subject}"] = 0
        st.session_state[f"attendance_{subject}"] = 0
        st.session_state[f"weight_{subject}"] = (
            st.session_state.subject_weights[subject]
        )

    # Remove inputs belonging to added subjects
    for key in list(st.session_state.keys()):

        if (
            key.startswith("grade_score_")
            or key.startswith("attendance_")
            or key.startswith("weight_")
        ):

            if not any(
                key == f"grade_score_{subject}"
                or key == f"attendance_{subject}"
                or key == f"weight_{subject}"
                for subject in [
                    "Mathematics",
                    "Science",
                    "English"
